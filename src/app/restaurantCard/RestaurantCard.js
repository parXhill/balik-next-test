...
export default function RestaurantCard({restaurant, dispatch}){
    ...
    return (
        <>
        <div onClick={handleClick} id={restaurant.id} className={`relative flex-none flex flex-col bg-slate-100 rounded-3xl mx-2 overflow-y-scroll no-scrollbar shadow-xl mb-2 transition-all duration-300 ${restaurant.id === selectedRestaurant ? "h-[25rem]  w-[35rem]" : "h-48 w-64"}`}>
                ...
                <div id="information" className={`text-xs py-2 px-5 flex-col ${restaurant.id === selectedRestaurant ? "flex" : "hidden"}`}>
