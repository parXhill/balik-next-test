...
const RestaurantCard = ({restaurant}) => {
    ...
    function DealTag({tag}){
        return(
            <p key={tag} className="capitalize inline-block bg-slate-300 p-1 text-xs rounded-xl mr-2 px-2">{tag}</p>
        )
    }

    return (
        <div key={restaurant.id} className="relative flex-none flex flex-col bg-slate-100 h-48 w-64 rounded-3xl mx-2 overflow-y-scroll no-scrollbar">
                ...
                <div className="ml-5 grid grid-rows-2 grid-cols-[4fr_1fr]">
                    ...
                    <div className="col-span-2 row-span-1">
                        {restaurant.deal[0].tags.map(tag => {return(<DealTag tag={tag}/>)})}
                    </div>
                </div>
